...
gulp.task('sass', function() {
  return gulp.src('styles/main.scss')
    ...
    .on('error', $.notify.onError({
      title: 'SASS Failed',
      message: 'Errors occurred during compilation.'
    }))
...
gulp.task('sass:build', function() {
  ...
  return gulp.src('styles/main.scss')
    ...
    .pipe($.uncss({
      html: ['./index.html', './scripts/components/**/*.html'],
      ignore: [
        '.index',
      ]
    }))
...
gulp.task('usemin', function() {
  return gulp.src('./index.html')
    // add templates path
    .pipe($.htmlReplace({
      'templates': '<script type="text/javascript" src="scripts/templates.js"></script>'
    }))
    .pipe($.usemin({
      styles: [$.minifyCss(), 'concat'],
      vendor: [$.uglify()],
      scripts: [$.uglify()]
    }))
    .pipe(gulp.dest('./_build/'));
...
gulp.task('default', ['browser-sync', 'sass', 'minify-css'], function() {
  ...
  gulp.watch(['*.html', 'scripts/components/**/*.html'], ['bs-reload']);
